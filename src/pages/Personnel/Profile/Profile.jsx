...
const Profile = () => {
  ...
  const loadPersonnelProfile = useCallback(async () => {
    setLoading(true);
    try {
      const apiBase =
      ...
      const response = await fetch(
        `${apiBase.replace(/\/$/, "")}/teacher/personnel/me?t=${Date.now()}`,
        {
          headers: {
      ...
      if (!response.ok) {
        showAlert.error(
          "Profile",
          data?.message ||
            "Unable to load your profile. Please contact your property custodian."
        );
        setPersonnel(null);
        return;
      }

      const fetchedPersonnel = data.personnel || data;
      setPersonnel(fetchedPersonnel);

      const contactPhone = fetchedPersonnel.phone
        ? formatContactPhone(fetchedPersonnel.phone)
        : "";

      const payload = {
        first_name: fetchedPersonnel.first_name || "",
        last_name: fetchedPersonnel.last_name || "",
        employee_id: fetchedPersonnel.employee_id || "",
        id_number: fetchedPersonnel.id_number || "",
        username: fetchedPersonnel.username || "",
        phone: contactPhone,
        position: fetchedPersonnel.position || "",
        employment_status: fetchedPersonnel.employment_status || "",
        employment_level: fetchedPersonnel.employment_level || "",
        subject_area: fetchedPersonnel.subject_area || "",
        rating: fetchedPersonnel.rating || "",
        notes: fetchedPersonnel.notes || "",
      };
      ...
      setHasChanges(false);
      const avatar = resolveAvatarUrl(fetchedPersonnel);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBase.replace(/\/$/, "")}/teacher/personnel/me`,
        {
      ...
      <div className="container-fluid px-2 px-md-3 py-4 fadeIn">
        <div className="text-center py-5">
          ...
          <p className="text-muted mb-0">
            Your account is not currently linked to a personnel record. Please
            contact your property custodian to set up your profile.
          </p>
        </div>
      </div>
    );
      ...
      <div className="text-center mb-4">
        <div className="d-flex flex-column flex-md-row justify-content-center align-items-center mb-3">
          ...
          <div className="text-center text-md-start">
            ...
            <p className="text-muted mb-0">
              {personnel?.employee_id || "No employee ID"} •{" "}
              {personnel?.position || "Position not set"}
            </p>
                    ...
                    <div className="col-12 col-md-8">
                      <div className="row g-3">
                        ...
                        <div className="col-md-6">
                          ...
                          />
                          <small className="text-muted">
                            Contact your property custodian to change
                          </small>
                        </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!hasChanges) {
                          ...
                          const avatar = resolveAvatarUrl(personnel);
                          updateAvatarPreview(avatar || "");
                          setAvatarFile(null);
                          setAvatarRemoved(false);
                          setErrors({});
